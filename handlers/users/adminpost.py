...
@dp.message_handler(IsPrivate(), chat_id=ADMINS , commands=['Sendpost'])
...
async def start_send_text(call: types.ContentTypes, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            pass
    await dp.bot.send_message(chat_id=ADMINS[0], text=f"Xabaringiz {senddingMSG} ta foydalanuvchiga muvofaqqiyatli yuborildi.")
